"""
Tests for SubjectRepository._count_subjects_by_diagnosis_category.
...
- public count_subjects_by_field dispatching to the private method
...
def mock_settings() -> Mock:
    ...
    s.subject_count_fields = ["associated_diagnosis_categories"]
...
@pytest.fixture
def repository(mock_session: AsyncMock, mock_allowlist: Mock, mock_settings: Mock) -> SubjectRepository:
...
async def test_count_diagnosis_category_basic(repository, mock_session):
    ...
    result = await repository._count_subjects_by_diagnosis_category({})
...
async def test_count_diagnosis_category_exhausted_retries(mock_sleep, repository, mock_session):
    ...
    with pytest.raises(Exception, match="failure 2"):
        await repository._count_subjects_by_diagnosis_category({})

    assert mock_sleep.call_count == 2


async def test_count_subjects_by_field_routes_to_diagnosis_category(repository, mock_session):
    """count_subjects_by_field('associated_diagnosis_categories', ...) delegates to private method."""
    mock_session.run.side_effect = [
    ...
    result = await repository.count_subjects_by_field("associated_diagnosis_categories", {})
...
async def test_count_diagnosis_category_multiple_values(repository, mock_session):
    """All returned diagnosis_category rows appear in result['values'] sorted by count desc."""
    categories = [
...
@patch("asyncio.sleep", new_callable=AsyncMock)
async def test_count_diagnosis_category_values_canonicalize_lower(
